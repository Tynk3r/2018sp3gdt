...
*/
/******************************************************************************/
Mesh* MeshBuilder::GenerateQuad(const std::string &meshName, Color color, float length, float texCoordStretch)
{
	Vertex v;
	std::vector<Vertex> vertex_buffer_data;
	std::vector<GLuint> index_buffer_data;
	...
	v.color = color;
	v.normal.Set(0, 0, 1);
	v.texCoord.Set(texCoordStretch, 0);
	vertex_buffer_data.push_back(v);
	v.pos.Set(0.5f * length, 0.5f * length,0);
	v.color = color;
	v.normal.Set(0, 0, 1);
	v.texCoord.Set(texCoordStretch, texCoordStretch);
	vertex_buffer_data.push_back(v);
	v.pos.Set(-0.5f * length, 0.5f * length,0);
	v.color = color;
	v.normal.Set(0, 0, 1);
	v.texCoord.Set(0, texCoordStretch);
	vertex_buffer_data.push_back(v);
	...
	mesh->texCoordStretch = texCoordStretch;
...
Mesh* MeshBuilder::GenerateTerrain(const std::string &meshName, const std::string &file_path, std::vector<unsigned char>& heightMap) {
	...
	for (unsigned z = 0; z < terrainSize; ++z)
	{
		for (unsigned x = 0; x < terrainSize; ++x)
		{
			...
			float heightL = ReadHeightMap(heightMap, x - 1, z);
			float heightR = ReadHeightMap(heightMap, x + 1, z);
			float heightD = ReadHeightMap(heightMap, x, z - 1);
			float heightU = ReadHeightMap(heightMap, x, z + 1);

			Vector3 normalVector = Vector3(heightL - heightR, 2.0f, heightD - heightU);
			normalVector = normalVector.Normalize();

			v.normal.Set(normalVector.x, normalVector.y, normalVector.z);
			v.pos.Set(static_cast<float>(x) / terrainSize - 0.5, scaledHeight, static_cast<float>(z) / terrainSize - 0.5);
			v.color.Set(scaledHeight, scaledHeight, scaledHeight);
			v.texCoord.Set((float)x / terrainSize * 8, /*1.f -*/ (float)z / terrainSize * 8);
			vertex_buffer_data.push_back(v);
		}
...
Mesh * MeshBuilder::GenerateSkyPlane(const std::string & meshName, Color color, int slices, float PlanetRadius, float AtmosphereRadius, float hTile, float vTile)
{
	...
	float tex_dt = 2.0f / (float)slices; //divide texture coords
	...
	for (int z = 0; z <= slices; ++z)
	{
		for (int x = 0; x <= slices; ++x)
		{
			...
			Vertex vee;
			vee.pos.Set(xdist, -height, zdist);
			vee.texCoord.Set(hTile*((float)x*tex_dt*0.5f), vTile*(1.0f - (float)z*tex_dt*0.5f));
			vee.color = color;

			vertexbufferdata.push_back(vee);
		}
	}
	for (int i = 0; i < slices; ++i)
	{
		for (int j = 0; j < slices; ++j)
		{
			...
			indexbufferdata.push_back(startvert);
			indexbufferdata.push_back(startvert + 1);
			indexbufferdata.push_back(startvert + slices + 1);

			indexbufferdata.push_back(startvert + 1);
			indexbufferdata.push_back(startvert + slices + 2);
			indexbufferdata.push_back(startvert + slices + 1);
		}
	}
	Mesh* mesh = new Mesh(meshName);
	...
	mesh->indexSize = indexbufferdata.size();
	mesh->mode = Mesh::DRAW_TRIANGLES;
	return mesh;
}

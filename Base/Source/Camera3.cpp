...
void Camera3::Init(const Vector3& pos, const Vector3& target, const Vector3& up, std::vector<unsigned char> &heightMap)
{
...
void Camera3::Update(double dt)
{
	...
	/*if (Application::IsKeyPressed(VK_SPACE))
	{
		...
		right.y = 0;
		right.Normalize();
		position.y += 1.f * CAMERA_SPEED * (float)dt;
		target.y += 1.f * CAMERA_SPEED * (float)dt;
	}
	...
	if (position.y > 120 + 350.f * ReadHeightMap(m_heightMap, position.x / 4000, position.z / 4000))
	{
		Vector3 view = (target - position).Normalized();
		Vector3 right = view.Cross(up);
		right.y = 0;
	...
	{
		//position.y = target.y = 120 + 350.f * ReadHeightMap(m_heightMap, position.x / 4000, position.z / 4000);
		Vector3 view = (target - position).Normalized();
		Vector3 right = view.Cross(up);
		right.y = 0;
		right.Normalize();
		float oPosY = position.y;
		float oTarY = target.y;
